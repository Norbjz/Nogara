...
function pintar() {
  for (let index = 0; index < tiktokers.length; index++) {
    ...
    var contenido = `
    <div class="card-head">
      <h2 class="name">${tiktokers[index].name}</h1>
    ...
    <div class="set">
      <p class="card-set">TIK-TOK</p>
    </div>

    <div class="text-box">
      <h3 class="monster-type">[${tiktokers[index].type}]</h3>
      <p class="description">${tiktokers[index].info}</p>
      <div class="stats">
        <p class="attack">ATK/ ${tiktokers[index].atk}</p>
        <p class="defense">DEF/ ${tiktokers[index].def}</p>
      </div>
    </div>
    ...
    <div class="bottom">
      <div class="bottom-info">
        <div class="card-edition">
          <p>12345678 <sup class="sup">1ST</sup> Edition</p>
        </div>

        <div class="card-creator">&#169; 1996 KAZUKI TAKAHASHI</div>
      </div>
      <div class="holo"></div> 
  </div>`;
    ...
    for (let index2 = 0; index2 < tiktokers[index].stars; index2++) {
      let stars = document.getElementsByClassName('stars')[index];
      console.log(stars);
      let singlestar =
        '<img class="star-img" src="img/star.png" alt="estrella" />';

      stars.innerHTML += singlestar;
    }

    let cardImg = document.getElementsByClassName('card-img')[index];
    cardImg.style.backgroundImage = `url(img/cards/${tiktokers[index].img})`;
  }
}
